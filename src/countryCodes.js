...
let countryString = `Afghanistan	AF	AFG	004
Albania	AL	ALB	008
Algeria	DZ	DZA	012
...
const countryArray = countryString.split(/\d\d\d/g).map(country => {
  let codes = country.slice(country.length-7,country.length-1);
  return {[codes.slice(3,codes.length)]:codes.slice(0,2)};
});

fs.writeFileSync('./countryDictionary', JSON.stringify(countryArray, null, 2), 'utf-8');
